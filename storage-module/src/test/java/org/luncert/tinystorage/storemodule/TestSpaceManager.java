...
public class TestSpaceManager {
  ...
  private final TsRuntime.TsRuntimeBuilder runtimeBuilder = TsRuntime.builder()
      ...
      .maxStoreSize(TsFileHeader.HEADER_SIZE + 1024)
      .maxFileSize(TsFileHeader.HEADER_SIZE + 1024) // 1kb
  ...
  public void testSimpleRecycle() {
    ...
    Assert.assertEquals(TsFileHeader.HEADER_SIZE + 64, bucket.size());
  }
  ...
  public void testComplexRecycle() {
    TsRuntime runtime = runtimeBuilder.maxStoreSize((TsFileHeader.HEADER_SIZE + 1024) * 2).build();
    SpaceManager manager = new SpaceManager(runtime);
    ...
    Assert.assertEquals(TsFileHeader.HEADER_SIZE + 1024, bucket0.size());
    Assert.assertEquals(TsFileHeader.HEADER_SIZE + 64, bucket1.size());
  }
}
